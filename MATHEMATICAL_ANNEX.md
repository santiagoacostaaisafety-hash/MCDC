...
- CED (Cincture Equatorial Dynamic): dynamic band formed by the intersection of the two cones. This band contains the east–zenith–west trajectories that represent the apparent motion of the sky. 
...
The CED defines the observable limit of the sky in the east–west direction. Its angular extent is defined by the intersection region between the two symmetric cones, but its physical scale is determined by the most distant objects visible at the extremes.
...
This value represents the maximum separation between two observable points in opposite directions along the CED band.
...
- In the model:
  - The vertex of the Polar Cone traces a circle (or closed trajectory) on the closed trajectory generated by the axial cone rotation over the precession cycle.
